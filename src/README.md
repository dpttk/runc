# runc (upstream documentation)

Upstream `runc` README kept for reference. **Build and run from the repository root** (`..`), not from `src/`. Fork-specific features (`--security-scan`, capability defaults) are documented in [../README.md](../README.md).
...
with some of them enabled by default (see `BUILDTAGS` in the top-level [`Makefile`](../Makefile)).
