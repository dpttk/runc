...
Campaign \texttt{campaign-20260618-220913} executed 22 reproducers (dangerous syscalls, capability-bound mounts, filesystem breakouts, post-RCE lateral movement, misconfigurations, scoped runtime-CVE checks, and a positive control). An attack is \emph{blocked} when it fails with seccomp/AppArmor signals, kernel audit \texttt{DENIED} records, or missing expected side-effects. Table~\ref{tab:security-corpus-ch5} summarises blocked/total counts.

\begin{table}[ht]
\centering
\caption{Kernel-boundary security effectiveness (campaign \texttt{20260618-220913}). Misconfiguration rows apply only to Docker-backed configs.}
