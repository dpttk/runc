...
\textbf{RQ1 --- Can a runtime, without infrastructure changes, generate profiles whose security level approaches manual tuning or cluster-side recorders?} On exercised paths, yes: the laboratory produced deny-by-default seccomp, complain-mode AppArmor, and cgroup-wide capability traces from a single \texttt{--security-scan} run (Chapter~\ref{ch:experiments}, Table~\ref{tab:profquality-lab}). File capabilities on executed binaries (e.g.\ \texttt{CAP\_NET\_RAW} on \texttt{ping}) are not yet merged from \texttt{security.capability} xattrs, and AppArmor path learning required parsing both \texttt{ALLOWED} and \texttt{DENIED} audit records --- limitations documented as drawbacks rather than hidden.

\textbf{RQ2 --- How should the runtime expose this capability so it remains a one-flag operation?} Three constraints turned out to be load-bearing: (i) host-supplied confinement must be relaxed in memory, not on disk; (ii) capability tracing must be cgroup-wide, otherwise child processes break the next run; and (iii) finalisation must be set-difference and never set-union. The implementation honours all three; integration smoke tests and the laboratory confirm that one flag produces the expected \texttt{generated/} layout without embedding scripts in the bundle.

\textbf{RQ3 --- How much does post-exploitation containment improve relative to stock \texttt{runc}?} On the tested workload, scan-then-enforce blocked sensitive reads, reverse shells, mount probes, and persistence attempts that succeeded under baseline stock \texttt{runc}, while preserving legitimate ping and calendar behaviour after documented \texttt{apply.sh} adjustments (Table~\ref{tab:security-lab}). Command injection itself remained possible by design; the value is blast-radius reduction after compromise.
...
\textbf{Broader laboratory coverage.} Additional intentionally vulnerable workloads and attack scripts would test whether containment results generalise beyond the Flask system-monitor bundle.
...
\textbf{Scientific publication.} The artefact and evaluation are intended to be distilled into a conference paper (e.g.\ LLNCS format) once the performance study and additional workloads are complete.
