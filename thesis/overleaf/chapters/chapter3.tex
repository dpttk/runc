...
The threat model assumes a co-tenant or compromised in-container process attempting one of these actions.
...
\begin{itemize}
    \item \emph{Web-application vulnerabilities without syscall escalation.} SQL injection confined to the database, cross-site scripting, insecure deserialisation, and broken authentication are out of scope for the scanner --- it does not patch application logic. OS command injection that reaches \texttt{execve} is exercised in the laboratory and is blocked by the generated seccomp profile when process-creation syscalls were not observed during the scan.
...
Command injection & succeeds & same & blocked (\texttt{fork}/\texttt{exec}) & depends & depends & strong isolation (lit.) \\
\midrule
...
\caption{Syscall path from application to host (after Jajoo~\cite{jajoo2025syscalls}) and qualitative mechanism use per confinement posture. Hardened enforcement prevents command injection because the generated seccomp profile omits process-creation syscalls the workload did not exercise during the scan.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{fig3_2_picril.png}
...
\textbf{Phase 4. Tracer shutdown.} The poststop \texttt{scan-cap-trace-stop} hook is prepended so that it runs before any user-defined or AppArmor-unload hook. It sends \texttt{SIGTERM} to the tracer, waits up to two seconds for the in-kernel ring buffer to flush, and falls back to \texttt{SIGKILL} if needed. The per-container BPF map is unpinned from the BPF filesystem. The AppArmor unload hook then collects audit records for the generated profile from \texttt{journalctl -k} or, if necessary, \texttt{dmesg}, and appends the learned rules to \texttt{generated/apparmor.profile}. The hooks together act as a barrier: by the time finalisation runs, the capability tracer has closed its log and the AppArmor profile contains the rules observed during the container lifetime.
...
\texttt{apparmor.profile} & AppArmor pipeline & Complain-mode stub plus audit-collected rules; flipped to enforce when rules exist \\
...
\texttt{.runc\_cap\_trace.pid}, \texttt{.runc\_aa\_started\_at} & internal hooks & Runtime state for tracer shutdown and AppArmor audit-window filtering \\
\bottomrule
\end{tabular}
\end{table}
...
On unload, the runtime reads kernel audit output, keeps only records for the generated profile and for the current scan window, accepts both denied and complain-mode allowed AppArmor events, and maps understood file and capability events into deterministic rules. Unsupported operations such as mount, signal, ptrace, or network events are kept as comments so that evidence is not lost. If AppArmor is disabled on the host, the profile file is still written for reference, but the hooks are not installed and an informational message is logged. The generated \texttt{apparmor-README.txt} still documents the operator workflow and optional \texttt{aa-logprof} review, but the runtime now performs the basic audit-to-rule conversion automatically.
