...
This section describes the evaluation methodology; the experimental setup and measured outcomes are reported in Chapter~\ref{ch:experiments}, and their interpretation in Chapter~\ref{ch:discussion}.
...
\textbf{Comparison method.} Security effectiveness is reported per attack class (reconnaissance, sensitive file read, reverse shell, mount probe, persistence) rather than as a single aggregated score, so that the reader can see which layer---seccomp, AppArmor, or capabilities---dominated each block. Performance comparison uses the same container images per workload across runtimes. Reproducer scripts, bundle definitions, and scan flags are versioned in the \texttt{runc-hardened-test} and \texttt{performance-evaluation} repositories.

\textbf{Reproducibility.} Every performance measurement runs ten times after one warm-up iteration, with the median reported. Security-effectiveness runs use a fixed payload script in the \texttt{runc-hardened-test} repository so that baseline and hardened responses can be compared byte-for-byte. The laboratory host is Ubuntu~24.04 with cgroup~v2 and AppArmor enabled.

\section{Summary}\label{sec:design-summary}

This chapter presented the design of the proposed runtime extension. The overall shape of the system is a two-mode runtime: a default mode that behaves as a drop-in \texttt{runc}, and a scanning mode that, behind a single CLI flag, runs the workload under relaxed confinement, records its behaviour through three independent tracers, and narrows the on-disk spec to the observed set. The design is driven by five requirements --- single-flag user experience, three profiles from one run, full \texttt{runc} compatibility, minimal dependency surface, and safe-by-construction finalisation --- and every decision in the three mechanism-specific sub-pipelines (capabilities, seccomp, AppArmor) follows from one of them. The threat model is deliberately narrow: the scanner protects the kernel-side perimeter against syscall-, capability-, and path-based escape attempts, and leaves the application-layer and supply-chain classes to controls one layer up. The implementation, experiments, and discussion are described in Chapters~\ref{ch:impl}--\ref{ch:discussion}.
