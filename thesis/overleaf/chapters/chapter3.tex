...
\textbf{R2. Three profiles from one run.} The scanner must produce, in a single invocation, three artefacts: (i)~a narrowed list of Linux capabilities, (ii)~an AppArmor profile learned in complain mode, and (iii)~a seccomp allow-list. The three artefacts come from independent sub-pipelines but share the same relaxation-and-recording skeleton, so that adding a new mechanism in the future requires only a new sub-pipeline, not a new scanner. SELinux is out of scope: the scanner may clear an existing \texttt{process.selinuxLabel} in memory during relaxation so that it does not mask the trace, but it does not generate a SELinux policy.
